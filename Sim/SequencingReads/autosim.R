...
if (is.tf) { # We do not simulate TF data, we kept this for completeness
...
dump2file <- function(id, cutoff, result) {
    ...
    write.table(file=result.file, data.frame(Method = id,
                                             Cutoff = cutoff,
                                             OneMinusPrecision = 1-result$overlap/result$found,
...
assessChIP2 = function (observed, known, time, tol = 200, checkfc = TRUE,gsize = gsize,is.error = F) 
{
    ...
    if(is.error==F){
        ...
        hitsperpeak <- as.data.table(all.olap)[,.N,by='queryHits'][,mean(N)]
        ...
        return(list(overlap = overlapped, found = found, recall = recall,
                    ...
                    tpr = sum(width(intersect(kranges,obranges)))/sum(width(kranges)),
                    fpr = sum(width(intersect(setdiff(gr.gensize,kranges),obranges)))/sum(width(setdiff(gr.gensize,kranges))),
                    ppv = sum(width(intersect(kranges,obranges)))/sum(width(obranges)),
                    error = 0))    
    } else{
        return(list(overlap = NA, found = NA, recall = NA,
                    time = NA,
                    callsize = NA,
                    peaksize = NA,
                    hitsperpeak = NA,
...
resultDump2 = function (regions, tab, cutoff = 0.05, out = "out.tsv",postprob = F,is.error = F) 
{
    if(is.error==F){
        if(postprob==T){
            ...
            sig <- mixNBHMM::fdrControl(prob = tab$Postprob,fdr = cutoff)
        } else{
            sig <- (tab$FDR <= cutoff)   
        }
        ...
        write.table(data.frame(chr = as.character(seqnames(sig.bins)),
                               start = start(sig.bins), end = end(sig.bins), tab[sig, ]),
                    file = out, row.names = FALSE, quote = FALSE, sep = "\t")
    } else{
        write.table(data.frame(chr = 'chrA',start = 0, end = 0),
                    file = out, row.names = FALSE, quote = FALSE, sep = "\t") 
    }
...
for (it in iters) {
    ...
    for (lib in 1:length(grouping)) {
        ...
        if (!is.tf) {
            ...
            peakFile(cname, chrs=chrs[drop.1], pos=pos.1[drop.1], mu=mean(back.mu), disp=rep(dispersion,sum(drop.1)),
                     sizes=sizes, fraglen=fraglen, width=true.width, tf=FALSE)
            peakFile(cname, chrs=chrs[drop.2], pos=pos.2[drop.2], mu=mean(back.mu), disp=rep(dispersion,sum(drop.2)),
                     sizes=sizes, fraglen=fraglen, width=true.width, tf=FALSE, append=TRUE)
            peakFile(cname, chrs=chrs[drop.3], pos=pos.3[drop.3], mu=mean(back.mu), disp=rep(dispersion,sum(drop.3)),
                     sizes=sizes, fraglen=fraglen, width=true.width, tf=FALSE, append=TRUE)
            ...
            # Generating Control reads
            peakFile(cname, chrs=chrs, pos=pos.1, mu=rep(mean(back.mu),length(cur.mu)), disp=rep(dispersion,length(disp)),
                     sizes=sizes, fraglen=fraglen, width=true.width, tf=TRUE)
        }
        fnames[[lib]]<-fname
        cnames[[lib]]<-cname
    ...
    addBackground(fnames, sizes=sizes, width=back.width, rlen=back.rlen,back.mu = back.mu,
                  dispersion=dispersion, prior.df=prior.df, append=TRUE)
    bam.files<-crunch2BAM(fnames)
    unlink(fnames)
    
    cnames<-unlist(cnames)
    addBackground(cnames, sizes=sizes, width=back.width, rlen=back.rlen,back.mu = back.mu,
                  dispersion=dispersion, prior.df=prior.df, append=TRUE)
    ...
    for (peakcaller in c("HOMER")) {
        ...
        if (peakcaller=="HOMER") {
            timeHOMER <- microbenchmark({
                ...
                for (x in 1:length(bam.files)) { 
                    ...
                    dir.create(pooldir)
                    poolcontroldir <- file.path(hpdir, prefix.control[x]) 
                    dir.create(poolcontroldir)
                    
                    system(paste("makeTagDirectory", pooldir, bam.files[x], "-format sam -keepAll"))
                    system(paste("makeTagDirectory", poolcontroldir, bam.control.files[x], "-format sam -keepAll"))
                    
                    outers <- file.path(peakdir, paste0("homer_", prefix[x], ".txt"))
                    system(paste("findPeaks", pooldir,"-i",poolcontroldir, "-style", ifelse(is.tf, "factor", "histone"), "-o", outers, "-gsize", gsize, "-fragLength", fraglen,"-inputFragLength",fraglen, "-tbp", 0))
        ...
        timeDiffBind <- microbenchmark({
            current <- dba(sampleSheet=data.frame(SampleID=prefix,
                                                  ...
                                                  PeakCaller=pktype), minOverlap=2)
            ...
            current <- dba.analyze(current, method=DBA_DESEQ2, bTagwise=!is.homo) # Avoiding tagwise.dispersion when data is homoskedastic.
        ...
        if(iters==1){
            write.table(as.data.frame(out),file = paste0(results,'DiffBind_ranges.txt'),row.names = F,quote = F,sep='\t')
            write.table(as.data.frame(curtab),file = paste0(results,'DiffBind_table.txt'),row.names = F,quote = F,sep='\t')
        }
        
        for (cutoff in fdr.thresholds) { 
            resultDump2(out, curtab, cutoff, out=ofile)
            result<-assessChIP2(ofile, lfile, tol=NA, checkfc=FALSE,gsize = gsize,time = timeDiffBind$time/convtime+timeHOMER$time/convtime) # Don't bother checking fold change, as it isn't well defined for complex events.
        ...
        timeChIPComp <- microbenchmark({
            for(peak in unlist(all.peakfiles)){
                write.table(fread(peak)[,1:3],file = gsub('.txt','.bed',peak),col.names = F,row.names = F,quote = F,sep = '\t')
            }
            ...
            dsg=as.data.frame(lapply(conf[,c("condition","factor")],as.numeric))-1
            dsg=as.data.frame(model.matrix(~condition,dsg))
            
            countSet = makeCountSet(conf,dsg,filetype="bam",binsize = binsize,species = 'other')
        ...
        if(iters==1){
            write.table(as.data.frame(gr.ChIPComp),file = paste0(results,'ChIPComp_ranges.txt'),row.names = F,quote = F,sep='\t')
            write.table(as.data.frame(dt.ChIPComp),file = paste0(results,'ChIPComp_table.txt'),row.names = F,quote = F,sep='\t')
        }
        
        for (cutoff in fdr.thresholds) { 
            resultDump2(gr.ChIPComp, dt.ChIPComp, cutoff, out=ofile)
            result<-assessChIP2(ofile, lfile, tol=NA, checkfc=FALSE,gsize = gsize,time = timeChIPComp$time/convtime+timeHOMER$time/convtime) # Don't bother checking fold change, as it isn't well defined for complex events.
    ...
    timediffReps <- microbenchmark({
        ...
        for (x in 1:length(bam.files)) {
            convertBamToBed(bam.files[x],file.path(drdir,gsub('./','',gsub('.bam','.bed',bam.files[x]))))
        }
        for (x in 1:length(bam.control.files)) {
            convertBamToBed(bam.control.files[x],file.path(drdir,gsub('./','',gsub('.bam','.bed',bam.control.files[x]))))
        }
        
        write.table(data.frame('chrA',gsize),file = paste0(drdir,'/gsize.txt'),col.names = F,row.names = F,quote = F,sep = '\t')
        
        bedfiles = list.files(drdir,pattern = 'hist.*.bed',full.names = T)
        bedcontrolfiles = list.files(drdir,pattern = 'control.*.bed',full.names = T)
        
        cmd = paste('diffReps.pl --chrlen ',paste0(drdir,'/gsize.txt'),
                    ' --report',paste0(drdir,'/diffreps.txt'),
                    '--treatment',paste(bedfiles[1:2],collapse = ' '),
                    '--control',paste(bedfiles[3:4],collapse = ' '),
                    '--btr',paste(bedcontrolfiles[1:2],collapse = ' '),
                    '--bco',paste(bedcontrolfiles[3:4],collapse = ' '),
                    '--pval 1',
                    '--nsd',ifelse(is.tf,'sharp','broad'),
                    '--window',binsize,
                    '--meth nb',
                    '--frag',fraglen,
                    '--nohs --noanno')
        ...
        tb.diffreps <- fread(paste0(drdir,'/diffreps.txt'),skip = 32)
        print(head(tb.diffreps))
        tb.diffreps[,FDR := padj]
        gr.diffreps <- with(tb.diffreps,GRanges(Chrom,IRanges(Start,End)))
    },times = 1)
    
    if(iters==1){
        write.table(as.data.frame(gr.diffreps),file = paste0(results,'diffReps_ranges.txt'),row.names = F,quote = F,sep='\t')
        write.table(as.data.frame(tb.diffreps),file = paste0(results,'diffReps_table.txt'),row.names = F,quote = F,sep='\t')
    }
    
    for (cutoff in fdr.thresholds) { 
        resultDump2(gr.diffreps, tb.diffreps, cutoff, out=ofile)
        result<-assessChIP2(ofile, lfile, tol=NA, checkfc=FALSE,gsize = gsize,time = timediffReps$time/convtime) 
    ...
    timeRSEG <- microbenchmark({
        ...
        # Pooling
        cmd = paste('samtools merge',paste0(drdir,'/histone_A.bam'),paste0(bam.files[1:2],collapse=' '))
        cat(paste('Command:',cmd,'\n'))
        system(cmd)
        
        cmd = paste('samtools merge',paste0(drdir,'/histone_B.bam'),paste0(bam.files[3:4],collapse=' '))
        cat(paste('Command:',cmd,'\n'))
        system(cmd)
        
        # Transforming to BED and sorting
        for (x in c(paste0(drdir,'/histone_A.bam'),paste0(drdir,'/histone_B.bam'))) {
            convertBamToBed(x,gsub('.bam','.bed',x))
            system('export LC_ALL=C')
            system(paste('sort -k1,1 -k3,3n -k2,2n -k6,6r',gsub('.bam','.bed',x),'-o',gsub('.bam','.sorted.bed',x)))
        }
        
        bedfiles = list.files(drdir,pattern = '*.sorted.bed',full.names = T)
        
        # Genome size and deadzones
        write.table(data.frame('chrA',1,gsize),file = paste0(drdir,'/gsize.txt'),col.names = F,row.names = F,quote = F,sep = '\t')
        write.table(data.frame('chrA',1,2),file = paste0(drdir,'/deadzones.txt'),col.names = F,row.names = F,quote = F,sep = '\t')
        
        # Calling RSEG
        cmd = paste('~/rseg-rseg-0.4.9/bin/rseg-diff -verbose -mode 3',
                    '-out',paste0(drdir,'/rseg_output.txt'),
                    '-score',paste0(drdir,'/rseg_postprob.txt'),
                    '-b',binsize,
                    '-chrom',paste0(drdir,'/gsize.txt'),
                    '-deadzones',paste0(drdir,'/deadzones.txt'),
                    '-duplicates',
                    '-fragment_length',fraglen,
                    bedfiles[1],bedfiles[2])
        cat('Command: ',cmd)
        system(cmd)    
    },times = 1)
    ...
    if(file.exists(paste0(drdir,'/rseg_postprob.txt'))){
        ...
        if(iters==1){
            write.table(as.data.frame(gr.rseg),file = paste0(results,'RSEG_ranges.txt'),row.names = F,quote = F,sep='\t')
            write.table(as.data.frame(tb.rseg),file = paste0(results,'RSEG_table.txt'),row.names = F,quote = F,sep='\t')
        }
        
        for (cutoff in fdr.thresholds) { 
            resultDump2(gr.rseg, tb.rseg, cutoff, out=ofile,postprob = T)
            result<-assessChIP2(ofile, lfile, tol=NA, checkfc=FALSE,gsize = gsize,time = timeRSEG$time/convtime) 
        ...
        for (cutoff in fdr.thresholds) { 
            resultDump2(gr.rseg, tb.rseg, cutoff, out=ofile,postprob = T,is.error = T)
            result<-assessChIP2(ofile, lfile, tol=NA, checkfc=FALSE,gsize = gsize,time = timeRSEG$time/convtime,is.error = T) 
            dump2file('RSEG', cutoff, result)
        }
    }
    unlink(drdir, recursive=TRUE)
    ...
    timecsaw <- microbenchmark({
        ...
        data <- windowCounts(bam.files, width=binsize, ext=fraglen, param=xparam, filter=20)
    ...
    if(iters==1){
        write.table(as.data.frame(merged$region),file = paste0(results,'csaw_ranges.txt'),row.names = F,quote = F,sep='\t')
        write.table(as.data.frame(tabneg),file = paste0(results,'csaw_table.txt'),row.names = F,quote = F,sep='\t')
    }
    
    for (cutoff in fdr.thresholds) { 
        resultDump2(merged$region, tabneg, cutoff, out=ofile)
        result<-assessChIP2(ofile, lfile, tol=NA, checkfc=FALSE,gsize = gsize,time = timecsaw$time/convtime) 
    ...
    timemixNBHMM <- microbenchmark({
        out.mixNBHMM <- mixNBHMM::mixNBHMMDataSetFromBam(bamFiles = bam.files,
                                                        colData = data.frame(Condition = c('1','1','2','2'),Replicate = c(1,2,1,2)),
                                                        genome = GenomicRanges::GRanges('chrA',IRanges::IRanges(1,gsize)),
                                                        blackList = NULL,
                                                        fragLength = rep(0,4),
                                                        windowSize = binsize)
        
        out.mixNBHMM <- mixNBHMM::mixNBHMM(out.mixNBHMM,control = mixNBHMM::controlEM())
        
        dt.mixNBHMM <- as.data.table(SummarizedExperiment::rowRanges(out.mixNBHMM))
        dt.mixNBHMM[,Postprob := metadata(out.mixNBHMM)$prob$Differential]
    },times = 1)
    
    if(iters==1){
        write.table(as.data.frame(SummarizedExperiment::rowRanges(out.mixNBHMM)),file = paste0(results,'mixNBHMM_ranges.txt'),row.names = F,quote = F,sep='\t')
        write.table(as.data.frame(dt.mixNBHMM),file = paste0(results,'mixNBHMM_table.txt'),row.names = F,quote = F,sep='\t')
    }
    
    for (cutoff in fdr.thresholds) { 
        resultDump2(SummarizedExperiment::rowRanges(out.mixNBHMM), dt.mixNBHMM, cutoff, out=ofile,postprob = T)
        result<-assessChIP2(ofile, lfile, tol=NA, checkfc=FALSE,gsize = gsize,time = timemixNBHMM$time/convtime) 
        dump2file("mixNBHMM", cutoff, result)
    }
    ...
    timeTHOR <- microbenchmark({
        ...
        write.table(data.frame('chrA',gsize),file = paste0(drdir,'/gsize.txt'),col.names = F,row.names = F,quote = F,sep = '\t')
        write.table(rbindlist(list(data.table(c('#rep1',bam.files[1:2])),
                                   data.table(c('#rep2',bam.files[3:4])),
                                   data.table(c('#inputs1',bam.control.files[1:2])),
                                   data.table(c('#inputs2',bam.control.files[3:4])),
                                   data.table(c('#chrom_sizes',paste0(drdir,'/gsize.txt'))))),file = paste0(drdir,'/THOR.config'),col.names = F,row.names = F,quote = F)
        
        cmd = paste('rgt-THOR',paste0(drdir,'/THOR.config'),
                    '--name peaks -b',binsize,
                    '--exts',paste0(rep(100,4),collapse = ','),'--pvalue 1.0 --output-dir',drdir)
        cat('Command: ',cmd)
        system(cmd)
        
        dt.thor = as.data.table(read.table(paste0(drdir,'/peaks-diffpeaks.narrowPeak'),header=F))
        dt.thor[,FDR := 10^(-V8)]
        print(head(dt.thor))
    ...
    if(iters==1){
        write.table(as.data.frame(gr.thor),file = paste0(results,'THOR_ranges.txt'),row.names = F,quote = F,sep='\t')
        write.table(as.data.frame(dt.thor),file = paste0(results,'THOR_table.txt'),row.names = F,quote = F,sep='\t')
    }
    
    for (cutoff in fdr.thresholds) { 
        resultDump2(gr.thor, dt.thor, cutoff, out=ofile)
        result<-assessChIP2(ofile, lfile, tol=NA, checkfc=FALSE,gsize = gsize,time = timeTHOR$time/convtime) 
