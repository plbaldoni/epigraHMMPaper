...
resultDump2 = function (regions, tab, cutoff = 0.05, out = "out.tsv",postprob = F,is.error = F) 
{
  if(is.error==F){
    if(postprob==T){
      ...
      sig <- epigraHMM:::fdrControl(prob = tab$Postprob,fdr = cutoff)
    } else{
      sig <- (tab$FDR <= cutoff)   
    }
...
for (it in iters) {
  ...
  for (peakcaller in c("HOMER")) {
    ...
    timeDiffBind <- microbenchmark({
      current <- dba(sampleSheet = data.frame(SampleID = prefix,Condition = grouping,
                                              bamReads = bam.files,ControlID = prefix.control,
                                              bamControl = bam.control.files,Peaks = unlist(all.peakfiles),
                                              PeakCaller = pktype),
                     minOverlap = 2)
      ...
      current <-dba.analyze(DBA = current,method = DBA_DESEQ2) # Avoiding tagwise.dispersion when data is homoskedastic.
  ...
  timeRSEG <- microbenchmark({
    ...
    # Calling RSEG
    cmd = paste('~/rseg/bin/rseg-diff -verbose -mode 3',
  ...
  timecsaw <- microbenchmark({
    ...
    bin.ab <- scaledAverage(binned, scale=median(getWidths(binned))/median(getWidths(data)))
  ...
  timeepigraHMM <- microbenchmark({
    ...
    dt.epigraHMM[, Postprob := exp(rhdf5::h5read(S4Vectors::metadata(out.epigraHMM)$output,
                                                 'logProb1')[, 2])]
  }, times = 1)
