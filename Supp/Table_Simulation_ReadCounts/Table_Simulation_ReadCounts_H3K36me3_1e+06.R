...
sumpar.subset <-
  sumpar[Marker == mark &
           Nwindow == size &
           Pct %in% seq(70, 100, 10),-c('Marker', 'Nwindow')]
sumpar.subset <-
  data.table::dcast(
    sumpar.subset,
    Pct + Groups + Parameter + True ~ Replicates,
...
sumpar.subset$Pct %<>% mapvalues(
  from = seq(70, 100, 10),
  to = c(
    paste0("Mean Ratio ",sumpar[Marker == mark & Pct == 70,.(mean(True)),by = 'Parameter'][Parameter %in% c('HMM.Mean.Int','HMM.Mean.Diff'),formatC(exp(V1[2]+V1[1])/exp(V1[1]),digits = 2,format = 'f')]," (70\\% SNR)"),
    paste0("Mean Ratio ",sumpar[Marker == mark & Pct == 80,.(mean(True)),by = 'Parameter'][Parameter %in% c('HMM.Mean.Int','HMM.Mean.Diff'),formatC(exp(V1[2]+V1[1])/exp(V1[1]),digits = 2,format = 'f')],"(80\\% SNR)"),
    paste0("Mean Ratio ",sumpar[Marker == mark & Pct == 90,.(mean(True)),by = 'Parameter'][Parameter %in% c('HMM.Mean.Int','HMM.Mean.Diff'),formatC(exp(V1[2]+V1[1])/exp(V1[1]),digits = 2,format = 'f')],"(90\\% SNR)"),
    paste0("Mean Ratio ",sumpar[Marker == mark & Pct == 100,.(mean(True)),by = 'Parameter'][Parameter %in% c('HMM.Mean.Int','HMM.Mean.Diff'),formatC(exp(V1[2]+V1[1])/exp(V1[1]),digits = 2,format = 'f')],"(100\\% SNR)")
  )
)

sink(paste0("Table_Simulation_ReadCounts_",mark,"_",size,".tex"))
sumpar.subset %>%
  ...
  kable(
    ...
    booktabs = T,
    escape = F,
    caption = paste0('Read count simulation. True values and average relative bias of parameter estimates (and $2.5^{th}$, $97.5^{th}$ percentiles) across a hundred simulated data sets are shown for ',mark,' with ',size,' genomic windows.\\label{Table:table_simulation}')
  ) %>%
  add_header_above(c(
    ' ' = 4,
    'One Replicate' = 2,
