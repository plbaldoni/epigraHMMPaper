...
fig1.250 = ggplot(data=ChIP[ChIP$Window%in%idx,],aes(x=start,y=Counts))+
  geom_line()+
...
fig2.250 = ggplot(data=PostProb[PostProb$Window%in%idx,],aes(x=start,y=PP,fill=Component))+facet_grid(rows=vars(Label))+
...
fig.250 = ggarrange(fig1.250,fig2.250,heights = c(0.8,0.2),ncol = 1,nrow = 2,legend = 'none')
save(fig1.250,fig2.250,fig.250,file = 'Figure_PeakCalls_H3K27me3_Encode_twocells_250bp.RData')