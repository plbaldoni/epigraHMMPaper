...
load(paste0('../../Data/Encode_hepg2/H3K27me3/wgEncodeBroadHistoneHepg2H3k27me3StdAlnRep1.markdup.q10.sorted.RData'))
hepg21 = subset(counts[[paste0(bp)]],chr==chromosome);rm(counts)
load(paste0('../../Data/Encode_hepg2/H3K27me3/wgEncodeBroadHistoneHepg2H3k27me3StdAlnRep2.markdup.q10.sorted.RData'))
...
idx.genome = c(49558274,50217318)
...
fig1.1000 = ggplot(data=ChIP[ChIP$Window%in%idx,],aes(x=start,y=Counts))+
  geom_line()+
  facet_grid(rows=vars(Group))+
  annotate('rect',alpha = 0.1,xmin = 49995000,xmax = 50155000,ymin = 0,ymax = 1.05*max(dt.segment$Counts,na.rm = T)) +
...
fig2.1000 = ggplot(data=PostProb[PostProb$Window%in%idx,],aes(x=start,y=PP,fill=Component))+facet_grid(rows=vars(Label))+
...
fig.1000 = ggarrange(fig1.1000,fig2.1000,heights = c(0.8,0.2),ncol = 1,nrow = 2,legend = 'none')
save(fig1.1000,fig2.1000,fig.1000,file = 'Figure_PeakCalls_H3K27me3_Encode_twocells_1000bp.RData')