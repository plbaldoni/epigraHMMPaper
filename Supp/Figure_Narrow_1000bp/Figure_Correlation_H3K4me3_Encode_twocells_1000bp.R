...
agg = function(x,
               gr.peak,
               gr.genome,
               cells = c('Helas3.adj', 'Hepg2.adj')) {
  gr.summarized = as.data.frame(GenomicRanges::findOverlaps(gr.peak, gr.genome))
  gr.summarized = as.data.table(cbind(gr.summarized, x[gr.summarized$subjectHits, cells, with =
                                                         F]))
  colnames(gr.summarized) = c('Peak', 'Genome', cells)
  gr.summarized[, Width :=  width(gr.peak[Peak])]
  gr.summarized <-
    gr.summarized[, .(
      Summary_Helas3.adj = sum(Helas3.adj, na.rm = T),
      Summary_Hepg2.adj = sum(Hepg2.adj, na.rm = T),
      Summary_Width = unique(Width)
    ), by = Peak]
...
getcor <- function(gr, p) {
  newgr <-
    gr[order(-abs(log2(Summary_Hepg2.adj + 1) - log2(Summary_Helas3.adj + 1))), ][1:round(.N *
                                                                                            p), ]
  newgr.UP = newgr[(Summary_Hepg2.adj >= Summary_Helas3.adj), ]
  newgr.DOWN = newgr[(Summary_Hepg2.adj < Summary_Helas3.adj), ]
  
  return(data.table(
    Up.LFC = ifelse(newgr.UP[, .N] > 1, newgr.UP[, median(log2((Summary_Hepg2.adj +
                                                                  1) / (Summary_Helas3.adj + 1)))], NA),
    Down.LFC = ifelse(newgr.DOWN[, .N] > 1, newgr.DOWN[, median(log2((Summary_Hepg2.adj +
                                                                        1) / (Summary_Helas3.adj + 1)))], NA),
    Cor = newgr[, cor(log2(Summary_Helas3.adj + 1),
                      log2(Summary_Hepg2.adj + 1),
                      method = 'spearman')]
  ))
}
...
dt.epigrahmm = agg(x = dt,
                   gr.peak = gr.epigrahmm,
                   gr.genome = gr.counts)
  ...
  tpr[['epigraHMM']] <-
    rbindlist(list(
      ...
      data.table(
        Method = 'epigraHMM',
        Top = 100 - 100 * i,
        getcor(gr = dt.epigrahmm, p = 1 - i)
      )
    ))
  # THOR
  tpr[['thor']] <-
    rbindlist(list(tpr[['thor']], data.table(
      Method = 'THOR',
      Top = 100 - 100 * i,
      getcor(gr = dt.thor, p = 1 - i)
    )))
  # csaw
  tpr[['csaw']] <-
    rbindlist(list(tpr[['csaw']], data.table(
      Method = 'csaw',
      Top = 100 - 100 * i,
      getcor(gr = dt.csaw, p = 1 - i)
    )))
  # RSEG
  tpr[['rseg']] <-
    rbindlist(list(tpr[['rseg']], data.table(
      Method = 'RSEG',
      Top = 100 - 100 * i,
      getcor(gr = dt.rseg, p = 1 - i)
    )))
  # diffReps
  tpr[['diffreps']] <-
    rbindlist(list(tpr[['diffreps']], data.table(
      Method = 'diffReps',
      Top = 100 - 100 * i,
      getcor(gr = dt.diffreps, p = 1 - i)
    )))
  # DiffBind
  tpr[['diffbind']] <-
    rbindlist(list(
...
out.summary.lfc <-
  melt(
    out.summary,
    id.vars = c('Method', 'Top'),
    measure.vars = c('Up.LFC', 'Down.LFC'),
    variable.name = 'Type',
    value.name = 'LFC'
  )

out.summary.lfc <- rbindlist(list(
  out.summary.lfc,
  data.table(
    Method = 'Genes',
    Top = 99.5,
...
fig.cor <- ggplot(data = out.summary, aes(x = Top)) +
  ...
  ylab('Corr. of H3K4me3 Counts') + xlab('Top (%) Peaks') +
  theme_bw() +
  ylim(-1, 1) +
...
save(fig.cor, file = 'Figure_Correlation_H3K4me3_Encode_twocells_1000bp.RData')
