...
idx.genome = c(19991497,20449507)
...
fig1.ezh2 = ggplot(data = ChIP[ChIP$Window %in% idx, ], aes(x = start, y =
                                                                  Counts)) +
  geom_line() +
  facet_grid(rows = vars(Group)) +
  annotate('rect',alpha = 0.1,xmin = 20136949-20*bp,xmax = 20295440+20*bp,ymin = 0,ymax = 1.05*max(dt.segment$Counts,na.rm = T))  +
  geom_segment(
...
fig2.ezh2 = ggplot(data = PostProb[PostProb$Window %in% idx, ], aes(x =
...
save(fig1.ezh2, fig2.ezh2, fig.ezh2, file = 'Figure_PeakCalls_EZH2_Encode_twocells_1000bp.RData')
