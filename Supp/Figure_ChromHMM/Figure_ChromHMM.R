...
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7","#6b023c","#8da7b5","#a8a46d")
names(cbPalette)[4:11] <- paste0('E',1:8)

chromhmm.out$States %<>% as.factor
chromhmm.out$Segment %<>% as.factor
...
fig_segmentation <- lapply(3:8,function(x){
  ggplot(data=ChIP[ChIP$Window%in%idx,],aes(x=start,y=Counts))+
    ...
    theme_bw() +
    scale_fill_manual(values = cbPalette[seq(4,11)],name = 'ChromHMM\nSegmentation',drop = FALSE)+
...
save(fig_segmentation, file = './Figure_ChromHMM.RData')

fig <- ggarrange(plotlist = fig_segmentation,nrow = 3,ncol = 2,labels = as.list(LETTERS[1:6]),legend = 'none')
fig <- ggarrange(get_legend(fig_segmentation[['8']]),fig,nrow = 2,heights = c(0.05,0.95))
ggsave(fig,filename = 'Figure_ChromHMM.pdf',height = 11.5,width = 9)
