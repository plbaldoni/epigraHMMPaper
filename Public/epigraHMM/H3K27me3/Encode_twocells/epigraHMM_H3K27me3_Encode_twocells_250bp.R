...
rdata <-
  c(
    ...
    'wgEncodeBroadHistoneHepg2H3k27me3StdAlnRep1.markdup.q10.sorted.RData',
    'wgEncodeBroadHistoneHepg2H3k27me3StdAlnRep2.markdup.q10.sorted.RData'
  )
mark <- 'H3K27me3'
method <- 'epigraHMM'
...
# epigraHMM
tryCatch({
  assign(paste(method, mark, type, 'Time', paste0(bin, 'bp'), sep = '_'),
         system.time({
           # Creating object
  ...
  saveerror(paste0(paste(
    method, mark, type, 'Error', paste0(bin, 'bp'), sep = '_'
  ), '.txt'), e)
}, error = function(e) {
...
do.call(save, list(
  paste(method, mark, type, 'Time', paste0(bin, 'bp'), sep = '_'),
...
tobed <- get(paste(method, mark, type, 'Output', paste0(bin, 'bp'), sep = '_'))
tobed <- callPeaks(object = tobed,
          hdf5 = file.path(diroutput,paste(method, mark, type, 'Output', paste0(bin, 'bp.h5'), sep = '_')),
          method = 'viterbi',
          saveToFile = FALSE)

rtracklayer::export.bed(object = tobed,con = file.path(diroutput,paste(method, mark, type, 'Bed', paste0(bin, 'bp.bed'), sep = '_')))
