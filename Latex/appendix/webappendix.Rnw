...
\hline
Cell Line & Control & H3K27me3 & H3K36me3 & EZH2 & H3K4me3 & H3K27ac & CTCF & RNA-seq\\
\hline
H1hesc & GSM733770 & GSM733748 & GSM733725 & GSM1003524 & GSM733657 & GSM733718 & GSM733672 & GSM758566\\
HelaS3 & GSM733659 & GSM733696 & GSM733711 & GSM1003520 & GSM733682 & GSM733684 & GSM733785 & GSM765402\\
Hepg2 & GSM733732 & GSM733754 & GSM733685 & GSM1003487 & GSM733737 & GSM733743 & GSM733645 & GSM758575\\
Huvec & GSM733715 & GSM733688 & GSM733757 & GSM1003518 & GSM733673 & GSM733691 & GSM733716 & GSM758563\\
\hline
\end{tabular}
\end{table}
...
In the chromatin segmentation analysis, ChromHMM was run with the commands:
\begin{itemize}
\item ChromHMM-BinarizeBed: \textit{java -jar $\sim$/ChromHMM/ChromHMM.jar BinarizeBam -b 500 -c 'controldir' ~/ChromHMM/CHROMSIZES/hg19.txt 'inputbamdir' 'input.txt' 'outputbinarydir'} and
\item ChromHMM-LearnModel: \textit{unset DISPLAY \&\& java -jar $\sim$/ChromHMM/ChromHMM.jar LearnModel -p 0 -b 500 'outputbinarydir' 'outputdir' 'numstates' hg19},
\end{itemize}

such that 'controldir' is the directory with control experiments, 'inputbamdir' is the directory with input bam files, 'input.txt' is an experimental table, 'outputbinarydir' is the output directory with binarized files, 'outputdir' is the model output, and 'numstates' is the number of states in the model. 

All ENCODE data analysis presented in the main text, and in this Web Appendix, were adjusted for input control samples in methods that are designed to do so. Specifically, differential peaks from ChIPComp, DiffBind, diffReps, and THOR were obtained on ChIP-seq samples while passing input control samples in their software call as an optional input argument argument. In contrast, input control samples were not considered in epigraHMM, csaw, and RSEG analyses.
...
In Figure \ref{fig:encode_gc_counts} below, we present smoothed scatter plots of log-transformed ChIP-seq counts (y-axis) and the respective GC content (x-axis) for the ENCODE EZH2 data that we analyzed. While using a 500bp window size to compute read counts, we did not observe enough evidence to justify account for such an effect in our analyses. Note that, when using such read count resolution, the effect of GC content can be hardly seen. In addition, there is no evident separation of differential GC-content effect for peak and background regions, as reported in \cite{teng2017accounting}.

\begin{figure}[h]
\begin{center}
\centerline{\includegraphics[scale = 0.75]{../../Supp/Figure_GC_Counts/Figure_GC_Counts.pdf}}
\end{center}
\caption{Smoothed scatter plots of log-transformed EZH2 ENCODE ChIP-seq counts and GC content.
\label{fig:encode_gc_counts}}
\end{figure}
